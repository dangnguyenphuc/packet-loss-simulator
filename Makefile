...
# make sure that you'd use these network interfaces
ATC_LAN_INTERFACE := wlp4s0
LAN_INTERFACE_SUBNETMASK := 10.42.0.1/24
ATC_WAN_INTERFACE := eno1
###################################################

ATC_RESTART_SLEEP_TIME := 20
ATC_ATCD_BINARY := $(HOME)/.local/bin/atcd
...
atc-reboot:
	...
	echo "LAN_INTERFACE_SM=$(LAN_INTERFACE_SUBNETMASK)"        	| sudo tee -a $(ATC_ENV_PATH) 		> /dev/null
